...
void ZyfHeap::post_initialize() {
  CollectedHeap::post_initialize();
}

void ZyfHeap::initialize_serviceability() {
  _pool = new ZyfMemoryPool(this);
  _memory_manager.add_pool(_pool);
}

GrowableArray<GCMemoryManager*> ZyfHeap::memory_managers() {
  GrowableArray<GCMemoryManager*> memory_managers(1);
  memory_managers.append(&_memory_manager);
  return memory_managers;
}

GrowableArray<MemoryPool*> ZyfHeap::memory_pools() {
  GrowableArray<MemoryPool*> memory_pools(1);
  memory_pools.append(_pool);
  return memory_pools;
}

size_t ZyfHeap::unsafe_max_tlab_alloc(Thread* thr) const {
  // Return max allocatable TLAB size, and let allocation path figure out
  // the actual allocation size. Note: result should be in bytes.
  return _max_tlab_size * HeapWordSize;
}

ZyfHeap* ZyfHeap::heap() {
  CollectedHeap* heap = Universe::heap();
  assert(heap != NULL, "Uninitialized access to ZyfHeap::heap()");
  assert(heap->kind() == CollectedHeap::ZYF, "Not an Zyf heap");
  return (ZyfHeap*)heap;
}
...
void ZyfHeap::collect(GCCause::Cause cause) {
  switch (cause) {
    ...
    case GCCause::_metadata_GC_clear_soft_refs:
      ...
      // While Zyf does not do GC, it has to perform sizing adjustments, otherwise we would
...
void ZyfHeap::safe_object_iterate(ObjectClosure *cl) {
  _space->safe_object_iterate(cl);
}

void ZyfHeap::print_on(outputStream *st) const {
  st->print_cr("Zyf Heap");
...
void ZyfHeap::print_tracing_info() const {
  print_heap_info(used());
  print_metaspace_info();
}

void ZyfHeap::print_heap_info(size_t used) const {
...
void ZyfHeap::print_metaspace_info() const {
